namespace June {
  // 연습문제 1
  const push = <T>(array: T[], element: T) => [...array, element] // 근데 이건 함수를 인자로 넘기는게 아니니까

  const modifyArray = <T>(array: T[], callback: (copy: T[]) => T[]) => callback([...array])
  const push2 = <T>(array: T[], element: T) => modifyArray(array, (copy) => copy.concat(element))

  const dropLast = <T>(array: T[]) => modifyArray(array, (copy) => copy.splice(0, copy.length-1)) // 책은 pop으로
  const dropFirst = <T>(array: T[]) => modifyArray(array, (copy) => copy.splice(1)) // 책은 shift로 구현
  // splice는 배열 내 요소를 전부 옮겨서 pop이나 shift가 더 효율적이다

  // 연습문제 2
  const modifyObject = <T>(object: T, callback: (copy: T) => T) => callback({...object})
  const deleteObject = <T>(object: T, key: keyof T) => modifyObject(object, (copy) => { delete copy[key]; return copy; })

  // 그치만 실제로는 이렇게 하겠지
  const deletedObject = <T>(object: T, key: keyof T) => { 
    const { [key]: _, ...rest} = object;
    return rest;
  }

  // 연습문제 3 - 현업에서 이런식으로 추상화해본적은 없는데...
  const tryCatch = <T extends unknown[]>(
  ...
  // 연습문제 4 - 이걸 누가 이렇게 쓰나
  const when = <T extends unknown[]>(
    condition: boolean, 
    callback: (...parameters: T) => void, 
    paramters: T, 
  ) => condition && callback(...paramters)

  // 연습문제 5 - 이걸 누가 이렇게 쓰나
  // 갑자기 함수명이 대문자로...? 
  const IF = <T extends unknown[], F extends unknown[]>(
    condition: boolean, 
    trueCallback: (...parameters: T) => void, 
    trueParamters: T, 
    falseCallback: (...parameters: T) => void, 
    falseParamters: T,
  ) => condition ? trueCallback(...trueParamters) : falseCallback(...falseParamters)
